    ...
    for directory in DATA_DIRS:
        ...
        try:
            if path.is_file() and path.stat().st_size:
                ...
                # Do not let a mounted but empty dashboard payload hide the
                # repository fallback.
...
for game in sorted(today_games, key=lambda g: str(g.get("time", "99:99"))):
    ...
    if p is None:
        ...
        hc = ac = ""
    match_rows.append(
        f"<tr><td>{esc(game.get('time'))}</td><td><b>{esc(home)}</b><span class='vs'>vs</span><b>{esc(away)}</b></td>"
...
for i, game in enumerate(ranked[:3], 1):
    try:
        ...
        prob = "-"
    top_rows.append(
        f"<div class='rank-row'><div class='rank-no'>{i}</div><div><b>{esc(game.get('pick'))}</b>"
        f"<small>{esc(game.get('home'))} vs {esc(game.get('away'))}</small></div>"
