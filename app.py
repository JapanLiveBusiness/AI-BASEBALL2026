...
with today_col:
    ...
    with g1:
        st.metric("対戦相手", npb["opponent"])

    with g2:
        st.caption("球場")
        st.markdown(
            f'<div style="font-size:1.55rem;font-weight:500;'
            f'line-height:1.2;white-space:normal;">'
            f'{npb["stadium"]}</div>',
            unsafe_allow_html=True
        )

    with g3:
        st.metric("開始", npb["time"])

    with g4:
        if npb["rank"] != "-":
            st.metric("パ・リーグ順位", f'{npb["rank"]}位')
        else:
    ...
    if npb["wins"] != "-":
        st.caption(
            f'🏆 シーズン成績：'
            f'{npb["wins"]}勝 {npb["losses"]}敗 '
            f'{npb["draws"]}分　勝率 {npb["pct"]}'
        )

    if npb["recent5"]:
        recent_text = "　".join(npb["recent5"])
        wins_recent = npb["recent5"].count("○")
        ...
        st.markdown(
            f"**直近5試合： {recent_text}　{recent_state}**"
        )

    if npb["ok"]:
        st.caption("✅ NPB公式データ自動同期")
    else:
        st.warning("NPB自動取得に失敗しました。手動設定で分析を継続します。")
