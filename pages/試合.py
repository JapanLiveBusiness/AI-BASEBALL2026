...
from research_state import current_slate, freshness, prediction_for_display
...
def starter_name(game: dict, side: str) -> str:
    keys = (
        ("home_starter", "home_pitcher", "starter_home")
        if side == "home"
        else ("away_starter", "away_pitcher", "starter_away")
    )
    for key in keys:
...
def starter_meta(game: dict, side: str) -> str:
    ...
    if wins is None and losses is None and era is None:
        return "成績データ取得待ち"
    return (
        f"{'－' if wins is None else wins}勝 "
        f"{'－' if losses is None else losses}敗 ｜ "
        f"防御率 {'－' if era is None else era}"
    )
...
next_date = slate.get("next_date")
switch_at = slate.get("switch_at")
is_previous_preview = bool(slate.get("is_previous_preview"))
prediction_payload = prediction_for_display(display_date)
...
health = freshness(schedule, now=now)
updated_at = schedule.get("updated_at") or "--"

if not games or featured is None:
    st.html(
        """
<style>
.empty-game-state{margin:26px 0;padding:34px;border:1px dashed rgba(255,255,255,.18);border-radius:16px;background:#0c1117;color:#a8b0ba;text-align:center}
</style>
<div class="empty-game-state">表示できる試合データがありません。次回データ同期後に自動表示されます。</div>
"""
    )
    st.stop()
...
feature_pred = pred_by_game.get((home_raw, away_raw), {})
pick_raw = str(feature_pred.get("pick") or "")
...
if is_final(featured):
    ...
    result_html = '<div class="premium-result"><span class="live-dot">● LIVE</span><span>試合速報更新中</span></div>'

if probability is not None and pick_raw:
    ai_html = f"""
<div class="premium-ai">
  <div class="premium-bot">🤖</div>
  <div>
    <div class="premium-ai-label">AI 勝率予測</div>
    <div class="premium-prob">{probability:.1f}%</div>
    <div class="premium-pick">注目チーム：{html.escape(pick_raw)}</div>
  </div>
  <div class="premium-track"><span style="width:{max(0.0, min(100.0, probability)):.1f}%"></span></div>
  <div class="premium-confidence">信頼度<b>{html.escape(confidence)}</b></div>
</div>
"""
else:
    ai_html = """
<div class="premium-ai">
  <div class="premium-bot">🤖</div>
  <div><div class="premium-ai-label">AI 勝率予測</div><div class="premium-prob muted">--</div><div class="premium-pick">分析データ待ち</div></div>
  <div class="premium-track"><span style="width:0%"></span></div>
  <div class="premium-confidence">信頼度<b class="muted">-</b></div>
</div>
"""
...
for game in games:
    if game is featured:
        continue
    game_home_raw = str(game.get("home") or "---")
    game_away_raw = str(game.get("away") or "---")
    game_venue = html.escape(str(game.get("venue") or "会場未定"))
    pred = pred_by_game.get((game_home_raw, game_away_raw), {})
    pred_chip = "AI分析待ち"
    if pred:
        p_pick = html.escape(str(pred.get("pick") or ""))
        p_prob = pred.get("win_probability")
        try:
            p_prob_text = f"{float(p_prob):.0f}%"
        except (TypeError, ValueError):
            p_prob_text = "--"
...
<article class="result-card">
  <div class="result-head"><span>{html.escape(str(game.get('time') or '--:--'))} · {game_venue}</span><span class="result-status{status_class}">{html.escape(status)}</span></div>
  <div class="result-match">
    <div class="result-team">{team_badge(game_away_raw, size='md')}<span>{html.escape(game_away_raw)}</span></div>
    <div class="result-score">{score_value(game.get('away_score'))} − {score_value(game.get('home_score'))}</div>
    <div class="result-team right"><span>{html.escape(game_home_raw)}</span>{team_badge(game_home_raw, size='md')}</div>
  </div>
  <div class="result-meta"><span>{pred_chip}</span><span>{html.escape(str(game.get('result_source') or 'NPB公式'))}</span></div>
</article>
"""
    )

other_games_html = (
    f'<div class="results-grid">{"".join(other_cards)}</div>'
    if other_cards
    else '<div class="empty-state">この表示日には他の試合カードはありません。</div>'
)

freshness_note = ""
if health.get("level") in {"stale", "warning"}:
    freshness_note = (
        f'<div class="data-note">データ状態：{html.escape(str(health.get("label")))} '
        f'（データ日 {html.escape(str(health.get("date"))) }）</div>'
    )
...
.preview-note,.data-note{{margin:10px 0 0;padding:10px 12px;border-radius:10px;font-size:.78rem}}
.preview-note{{background:#111b25;border:1px solid #28435d;color:#b9d8f5}}.preview-note strong{{color:#fff}}
.data-note{{background:#33290d;border:1px solid #775d13;color:#e9cf73}}
...
.premium-score{{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;min-height:210px;padding:30px 38px 22px}}
...
.premium-result{{display:flex;justify-content:center;gap:24px;padding:13px;border-top:1px solid #eef0f2;color:#1b2028;font-weight:900}}.live-dot{{color:#d72632}}
...
.premium-ai-label{{font-weight:850;color:#1a2432}}.premium-prob{{color:#2daf68;font-size:2.8rem;font-weight:950;line-height:1.05}}.premium-pick{{color:#687383;font-size:.78rem;font-weight:800}}.muted{{color:#7a8490!important}}
...
.premium-foot{{display:flex;justify-content:space-between;padding:7px 8px 2px;color:#aab2bc;font-size:.74rem}}
.results-section{{margin:22px 0 24px}}.results-title{{display:flex;align-items:center;gap:9px;margin-bottom:12px;font-size:1.08rem;font-weight:950;color:#fff}}
...
.ai-chip{{display:inline-flex;align-items:center;gap:6px;padding:5px 8px;border-radius:999px;background:#eef8f2;color:#16854c;font-weight:900}}.empty-state{{padding:28px;border:1px dashed rgba(255,255,255,.18);border-radius:14px;color:#9ca8b6;text-align:center;background:#0c1117}}
@media(max-width:900px){{.premium-score{{grid-template-columns:1fr;gap:18px;text-align:center;padding:24px 20px}}.premium-team,.premium-team.right{{justify-content:center;text-align:center}}.premium-team.right{{flex-direction:row-reverse}}.premium-ai{{grid-template-columns:auto 1fr}}.premium-track,.premium-confidence{{grid-column:1/-1}}.premium-details{{grid-template-columns:1fr}}.premium-detail{{border-right:0;border-bottom:1px solid #e8ebee}}.premium-starters{{grid-template-columns:1fr}}.results-grid{{grid-template-columns:1fr}}}}
...
<div class="results-section">
  <div class="results-title"><span>⚾</span>NPB 試合一覧</div>
  {other_games_html}
</div>
"""
