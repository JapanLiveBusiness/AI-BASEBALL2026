...
def require_login() -> None:
    """Require Auth0 OIDC login and restrict access to the configured email allowlist."""
    try:
...
<div class="login-shell"><div class="login-brand"><div class="login-logo">⚾</div><div><div class="login-title">AI BASEBALL STUDIO</div><div class="login-sub">AUTH0 PRIVATE LOGIN</div></div></div><div class="login-copy">この研究サイトを閲覧するにはAuth0ログインが必要です。</div></div>
""",
            unsafe_allow_html=True,
        )
        if st.button("Auth0でログイン", type="primary", use_container_width=True):
            try:
                st.login("auth0")
            except Exception:
                st.error("Auth0設定がまだ完了していません。管理者が接続情報を設定してください。")
        st.stop()
...
def current_username() -> str:
    try:
        return _user_email() or str(st.user.get("name") or "Auth0 user")
    except Exception:
        return "Auth0 user"
